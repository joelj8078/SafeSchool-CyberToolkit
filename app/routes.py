...
def generate_report():
    ...
    filepath = os.path.join("data", report_type, json_file)
    if not os.path.exists(filepath):
        return f"Error: {report_type.title()} JSON file not found.", 404

    if report_type == "network":
        generate_network_report(filepath)
    elif report_type == "web":
        generate_web_report(filepath)
